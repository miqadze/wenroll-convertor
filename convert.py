...
try:
    ...
    print(f"Error occurred while running upload_file(jpeg) on development with error: {e}")
try:
    subprocess.run(['/usr/bin/ffmpeg', '-i', input_file, '-vcodec', 'libx264', '-crf', '25', '-preset', 'medium', '-tune', 'film', '-y', output_file])
except Exception as e:
